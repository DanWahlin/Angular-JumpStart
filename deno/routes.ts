...
router.post('/api/customers', (ctx) => {
    let postedCustomer = ctx.request.body as any;
...
router.put<{id: string}>('/api/customers/:id', (ctx) => {
    let putCustomer = ctx.request.body as any;
...
router.post('/api/auth/login', (ctx) => {
    var userLogin = ctx.request.body;
